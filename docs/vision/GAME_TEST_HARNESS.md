...
- **Arma 3's pose cannot see the aim's pitch, so `level_pitch` cannot fix it.** A stick step that pitched the view by 8.6 degrees (measured against a target the mission publishes) left the pose's pitch field at zero, so the helper that levels the view read zero and did nothing, and checks that began by fronting the window measured a snap that was mostly pitch error. Anything that needs the view level on this game has to use a reference the engine agrees with: the assist checks pitch by the target's own published elevation until the engine draws it.
- **The clipboard pose channel misses on its own, not only when someone copies.** Two primitive checks in one run of six read "no pose" with nobody touching the machine. The retry that follows a miss is a read, not a repair, so a check that needs two poses either side of a move can still lose one; the numbers it produces are right or absent, never wrong.
- **One run in six died in the screen grab.** `env.step`'s `grabWindow` needs the Qt thread, and on one Arma 3 run it did not answer inside the ten seconds the hop allows, which ends the run. Nothing in the assist work touches that path; it is recorded because a run that dies at check four is not a failing suite, it is a rerun.
