...
> **VM reassessment (2026-09-12):** [Virtual Machine Feasibility](VIRTUAL_MACHINE_FEASIBILITY.md) rechecks the VM conclusions in section 4 (options C and D), section 5 (L2) and section 6, on a Windows host. Two arguments below were stated more broadly than the evidence supports: that every anti-cheat refuses every VM, and that the set of titles which both need Windows and tolerate a VM is nearly empty. Each needs a per-title check rather than a blanket rule, and the harness can now run one. The ranking does not change, for a reason that postdates the original text: section 8.4 measured the mouse filter doing the isolation job on one machine (against the probe's fake Raw Input window, with no stream in the path; its own latency was not measured), so a guest now has to beat a working local answer rather than an absent one. The tooling for that document's gates was built and run on 2026-09-13 (`vm/`, its section 10): Gate A is answered from published policy, Gates B and C passed on the dev machine (the RX 6600 XT partitions and renders in the guest; host input produced zero guest events through Moonlight and Sunshine while the pad arrived intact), and Gate D's latency axis was measured the same evening: input to photon through the guest is two to three display frames at the median and three to five at the 95th percentile, against one natively, so the ranking here does not change; a real game in the guest and a second machine were not measured.

> **Problem statement (2026-09-14):** [Input Ownership: Problem Statement](PROBLEM_STATEMENT.md) restates the problem this document researches, lists every boundary from game settings to a second computer with the evidence gathered here and since, and sets one yardstick for choosing between them.
...
| `vjoy_interface.py` (pyvjoy, requires vJoy driver) | **Shipped:** `UInputJoystickInterface` in `src/uinput_interface.py`, a pure-Python `uinput` device with 8 axes and 56 buttons. |
| `vigem_interface.py` (`src/padbus_client.py`, requires ViGEmBus) | **Shipped:** `UInputXboxInterface`, a `uinput` Xbox 360 pad with the xpad VID/PID, built by the factory `ControllerOutput` is given. **Verified end to end**, see the update note above. Windows went to its own bus client on 2026-09-09, so the two sides are siblings rather than one inheriting the other. |
