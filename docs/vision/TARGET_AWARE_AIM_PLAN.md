...
**Status:** 2026-09-13. The decision in section 2 was taken (yes, phases 0 to 2), phase 0 (the policy and the words) and phase 1 (the closed loop, measured in a real game) are built, and phase 2 has not started. Section 10 is the order of work and carries what each phase found.
...
*Built in phase 1 (`src/spectator/servo.py`). The first three points are as they were proposed; points 4 and 5 are what the simulation forced, and are the phase's main engineering result.*

The plant is an integrator with dead time: the stick commands an angular rate, the game integrates it into an angle, and the assist sees the result 60 to 120 ms later.

1. **Pixels to degrees.** A pinhole: `focal_px = (W / 2) / tan(hfov / 2)`, and a point `p` pixels off centre is `atan(p / focal_px)`. The harness measures `focal_px` for a game rather than trusting a menu (the T1 check), and measures the vertical scale separately, because a game's screen coordinates need not use the same span for both axes (Arma 3's do not).
2. **Degrees per second to magnitude.** The calibration tables give yaw against hold per magnitude. The steady rate is the longest hold present. `Servo.magnitude_for_rate(deg_per_s)` inverts that monotone table by interpolation, clamps to the largest calibrated magnitude and to the user's ceiling, and returns zero for a rate under the game's own deadzone rather than a floor value, so the assist never sends "just above the deadzone" into a game that has a cliff there (Halo Wars, Half-Life 2 in `AIM_ASSISTANCE.md` section 12.3). The cost of that rule is a floor on how finely the loop can aim, `min_rate / Kp` degrees, which is `Servo.settle_floor_deg` and which every "settled within" figure has to be read against.
3. **Gain against dead time.** The only period a loop with dead time can be reasoned about in is the dead time itself, because that is how long a command takes to become visible, so the gain is written as the fraction of the visible error to command away in one of them: `Kp = kp_fraction / τ`, 0.6 by default. That is well inside the `π / (2τ)` stability bound for an integrator with delay, and it converges by at least a third of the remaining error per dead time even if the calibrated table is 30 percent wrong either way.
4. **The plant model, because proportional alone does not meet the gate.** A proportional loop slow enough not to overshoot through 100 ms of dead time takes over a second to settle a 30 degree snap, which fails the plan's own 600 ms. So the servo subtracts the turn it has already commanded and not yet seen, integrated from the same calibration table: a Smith predictor whose model is the calibration. Two rules keep it safe. It may never claim a turn *past* the target, because only a measurement may say the view has overshot (without that clamp, a game that ramps into a turn makes the servo command a reversal, which subtracts from the in-flight total, which asks for the original direction again: a chatter at the period of the dead time that left 24 degrees of error on a 30 degree snap in simulation). And the dead time it is told should be an over-estimate, since over-stating it costs settling time while under-stating it overshoots by roughly the difference.
5. **The target's own rate, fed forward.** A proportional term alone leaves a standing error of `ω / Kp` on a target crossing at `ω`. The servo estimates `ω` without being told it: the error moves at the target's rate less the camera's, and the camera's is the servo's own command, delayed, which it knows. Three details, each of which was a measured failure first: the difference is taken on the error as measured rather than the filtered one (the filter's lag leaves part of the camera's own turn in the estimate, and a servo that reads its own motion as the target's chases itself); the baseline is a quarter of a second rather than a tick (a tick-long baseline is mostly the quantisation of when a command landed); and the estimate is believed only while the view is nearly still, because while it slews the estimate is dominated by whatever the plant model gets wrong. No integral term: a steady offset of a few pixels is preferable to windup against a target that steps away.

`tests/test_assist_servo.py` is where all of this is checked, against a simulated plant driven by the real Arma 3 calibration: a 15 degree snap settles in about a third of a second with no overshoot, and the loop still converges when the plant is 30 percent off its table, has half the delay it was told, or ramps into every turn.
...
2. **Phase 1, the closed loop (two to three days).** `servo.py` and its tests; `ScriptTargetSource` over the Arma 3 clipboard channel with the mission extended to publish a target; `look_at` in the runner; `get_spectator` as a slot; the T1 px/deg check and T2 to T8 in the harness; the calibration JSON added to the PyInstaller spec. Exit: T2 and T3 pass on `arma3_nobe`, and `arma3` with BattlEye running behaves identically, which is the same control the pad client used.
   ...
   *Done 2026-09-13, in a day.* All of it shipped, plus the user's own stick cancelling a running snap through `setStickInput`, and 36 of 36 checks pass on `arma3_nobe`. The numbers and everything the runs taught are in [GAME_TEST_HARNESS.md](GAME_TEST_HARNESS.md) section 8; the summary is that the answer to this phase's question is yes, and that it took more than the proportional loop section 7.3 originally described:

   - **The loop works.** A target 5, 15 and 30 degrees off centre is reached in 234, 265 and 422 ms, left within 0.14 to 0.26 degrees, with no overshoot at any angle, and a unit walking across at 1 and 2 m/s is tracked the same way. Every fail-safe holds in the real game: silence with a target on screen, the user's own stick handing control back with the driver left holding the user's vector, a deleted target decaying the command to zero in 188 ms, the Ctrl+Alt+F12 path releasing inside a tick, and the stick ceiling never exceeded.
   - **A proportional loop was not enough**, and a simulation said so before any game was launched (`tests/test_assist_servo.py`). The calibration had to become a plant model, and the target's own rate had to be estimated in a way that does not read the camera's own motion as the target's. Section 7.3 now describes what was built.
   - **The dead time was the surprise.** Against a script oracle the loop sees a command land in 15 ms, not the 60 to 120 ms the plan budgeted for a loop that watches a rendered frame, because the oracle reads the game's state rather than its picture. That is the one number phase 2 cannot inherit: its capture path will meet the plan's figure, the gain will drop with it, and the same snap will take proportionally longer. Everything else here carries over.
   - **The BattlEye control was not run, on purpose.** The plan's exit criterion asked for the `arma3` recipe to behave identically, but `policy.py` refuses target-aware assistance whenever an anti-cheat process is present, which is section 5's rule and which phase 0's own `arma3.json` entry already recorded. So the BattlEye recipe stops at the T0 gate with the refusal recorded and nothing target-aware executes; that refusal is the control. The pad-level comparison with and without BattlEye stands from 2026-09-09 and nothing in this phase changes it.
...
**What the highlight is, and what fires it.** Both were chased further the same evening, the first through the game's files and the second by running it.

It is a pulse, not a persistent tint. The duration is a slider with its own minimum, maximum and default (1.4 s); the colour is a gradient the highlight travels down over that duration; the code that runs it is a coroutine (`<HighlightDirt>d__254`) beside a flag called `IsFlashingOrHighlighting`; and its settings are built by `TabPageGeneral` next to the crosshair and stance dropdowns, so the row sits on the General tab of the options screen and reads "Show Dirt Highlight" with Color and Duration beneath it.

What fires it is still not known, and two runs of the game say what it is not. A sweep of every pad button that is safe to press in a job (A, B, X, Y, both bumpers, both stick clicks and the d-pad; never the triggers, which spray, and never Back or Start, which open menus) produced no coloured transient at all, twice, against an idle floor of 1.2 percent of the picture. Nor does it fire on its own: twenty seconds of standing still never changed the picture by more than 1.3 percent. There is also no symbol in the game's metadata that looks like a bindable "highlight dirt" action. So a Nimbus widget, which can only press a pad, cannot ask for it today.

The honest limit on that: both runs faced a fence panel the game's own HUD reports as 67 percent clean, so there was dirt on the object in view but not provably in the visible part of it, and a pulse over nothing visible is invisible. What would settle it is the options screen itself (does that row carry a keybinding?) or a run that first faces a known-dirty surface.

**The useful by-product.** Looking for dirt to face turned up a better oracle than a detector: the HUD's top-left panel names the wash target under the crosshair ("Fence Panel") and draws a per-material cleanliness bar beside it ("WOOD", 67.3 percent full as measured off the pixels), both at a fixed position in the frame. That is per-object ground truth, on screen, for free, needing no dirt detection of any kind: a harness could use it to ask "is the reticle on something dirty, and how dirty" in a second game without a script channel, and a user is already being told what they are pointing at. Two smaller facts from the same runs: on the pad, A is jump and the right stick click toggles stance, which is why both showed up as large picture changes before they were identified.
